...
Status: Gameplay Rules v0.2 authoritative contracts. Rules live in [GAME_RULES_MVP.md](GAME_RULES_MVP.md).
...
| PlayerState | Domain | PlayerId, Influence (0–3), Power (0–3), activeScheme (`"ambush" \| "bulwark" \| undefined`). Alive is derived from Influence; no independent mutable alive flag. |
| PendingStrike | Domain | Attacker, target, threat (1–3), force (0–3); only present in REACTION. |
| MatchView | Application contract | Whitelisted public match fields plus only the requesting member's private pending choice, private Power balance (opponents' Power is omitted/undefined), private activeScheme (opponents' scheme is hidden or represented as boolean `hasScheme: boolean`), and legal intent descriptions. |
...
Typed errors include WrongPhase, NotYourTurn, InvalidTarget, InvalidThreat, InvalidForce, InsufficientPower, PowerAtCap, InvalidReaction, InvalidScheme, and StalePhase. Transport errors such as Unauthenticated, StaleRevision, InvalidSettings, NotHost, and CommandIdConflict belong to application/protocol, not game rules.
...
Pending force, scheme types, and opponent Power balances are protected by view projection (`projectMatchView`, `projectEventsForViewer`), not domain filtering. Domain produces authoritative transitions with full state; the application layer masks opponent Power (`power?: undefined`), opponent activeScheme (`schemeType` hidden; only `hasScheme: boolean` visible), and secret commitments before delivery across HTTP snapshots, SSE streams, and event logs.

## Determinism and invariants

RandomProvider is a domain-owned interface: `nextInt(randomState, exclusiveMax) -> { value, nextState }`. It must be a pure operation on explicit state. A pinned local seeded implementation lives in domain/random; tests may inject scripted draws. Setup is the only consumer initially.
